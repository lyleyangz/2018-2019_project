<template>
    <div>
                ...
                <div class="pay_top">
                    <div class="item">
                      <div class="row row_three">
                          ...
                          <span class="cnt">{{Number(userLeftVaac || 0).toFixed(8)}}</span>
                      </div>
                    </div>
                    <div class="item">
                    ...
                    <div class="item">
                     ...
                     <div class="item" v-if="Number(payData.payMoney)>0">
                      <div class="row row_two">
                          ...
                          <input type="number"  class="ivu-input" v-model="useVAAC"  @blur="countPayWorth()">
                      </div>
                    </div>
                    <div class="item" v-if="Number(payData.payMoney)>0">
                      <div class="row row_two">
      ...
      <!-- 支付成功 -->
        <div class="pay_suc" v-if="pay_success&&order_type == 1">
            ...
            <div class="pay_content">
                ...
                <div class="pay_mid">
                    ...
                    <div class="bottom" v-if="pay_success_tra">{{pay_success_tra}}</div>
                </div>
                <div class="btn">
...
import { payOrder, queryCount,queryVaac } from "@api/api_shopping";
...
export default {
	...
	data() {
		return {
			...
			pay_detail: true,
			// 订单ID
			newProductID: this.$route.params.id,
			// 礼品订单
			giftID:null,
			payData: null,
			// 用户剩余通证
	...
	created() {
		...
		// this.payData = null;
		this.payData = JSON.parse(sessionStorage.getItem("TOPAYDATA"));
	...
	mounted() {
  	...
  	watch: {
		useVAAC(newVal, oldVal) {
            ...
            this.max.vaacCut = Number(this.useVAAC*(this.payData.sysm.vaacCut)).toFixed(8);
            this.max.maxUse = Number(this.useVAAC*(1+this.payData.sysm.vaacCut)).toFixed(8);
			// 折扣
	...
	methods: {
		...
		pay() {
			if (this.payVal) {
				payOrder({
					...
					oStatus: 2,
                    consumeToken:(this.payData.tokenPay),
                    giveToken: Number(this.payData.sysm.giveToken * this.payData.payMoney).toFixed(6),
                    vaacAccount:(Number(this.useVAAC)+Number(this.useVAAC)*this.payData.sysm.vaacCut || 0).toFixed(8),
					passphrase:this.payVal+'',
					// 
                    temporaryToken:this.payData.temporaryToken
				}).then(res => {
					if (res.code == "200" && res.msg == "success") {
						if(res.data.giftId){
							this.giftID = res.data.giftId;
						}
						Toast("支付成功!");
						this.pay_success_tra = res.data.traceno
		...
		orderDetail(){
			this.$router.replace({
				path: "/order/detail/"+this.newProductID
			});
		}
	},
	beforeDestroy(){
