...
class jigsaw {
    ...
    draw(x,y) {
        ...
        this.x = x//getRandomNumberByRange(L + 10, w - (L + 10))  // 50~248
        this.y = y//getRandomNumberByRange(10 + r * 2, h - (L + 10))// 0~110
    ...
    bindEvents() {
        ...
        document.addEventListener('mouseup', (e) => {
            ...
            this.trail = trail
            const {spliced, TuringTest,tag} = this.verify();
            
            // if (spliced) {
                if (TuringTest && spliced) {                    
                    addClass(this.sliderContainer, 'sliderContainer_success')
                    this.success && this.success(tag.x,tag.y)
                } else {
                    addClass(this.sliderContainer, 'sliderContainer_fail')
                    this.text.innerHTML = '再试一次'
                    this.fail && this.fail(_this)
                }
            // } else {
            //     addClass(this.sliderContainer, 'sliderContainer_fail')
            //     this.fail && this.fail()
...
var slideVerify = {};
var obj = null;
slideVerify.init = function(element,x,y, success, fail){
   return new jigsaw(element, success, fail).init(x,y)
}
slideVerify.reset = function(x,y){
    obj.init(x,y)
}
